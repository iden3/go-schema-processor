...
func TestValidator_ValidateDataNoTypeInService(t *testing.T) {
	...
	assert.ErrorContains(t, err, "missing properties: 'type'")
...
func TestValidator_ValidateDraft07(t *testing.T) {

	v := Validator{}
	err := v.ValidateData([]byte(cred07), []byte(schema07))
	require.NoError(t, err)
}

func TestValidator_ValidateDraft2020(t *testing.T) {

	v := Validator{}
	err := v.ValidateData([]byte(cred20), []byte(schema2020))
	require.NoError(t, err)
}
